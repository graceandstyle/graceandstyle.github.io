...
export default function App() {
  ...
  //const productsFinal = error ? fallbackProducts : products;
  const productsFinal = fallbackProducts;

  function updateCurrentCategoryID(categoryID) {
    setCurrentCategoryID(categoryID);
    setFilteredProducts(productsFinal.filter((p) => p.CategoryID === categoryID));
    setCurrentItemID('');
  }

  function updateCurrentItemID(itemID) {
    setCurrentItemID(itemID);
    setFilteredItem(filteredProducts[0].Items.filter((p) => p.ItemID === itemID));
    setCurrentVariationID('');
  }

  function updateCurrentVariationID(variationID, imgURL) {
    setCurrentVariationID(variationID);
    setFilteredVariation(filteredItem[0].Variations.filter((p) => p.VariationID === variationID));
    setImgURL(imgURL);
    setCurrentSizeID('');
  }

  function updateCurrentSizeID(sizeID) {
    setCurrentSizeID(sizeID);
    setFilteredSize(filteredVariation[0].Sizes.filter((p) => p.SizeID === sizeID));
  ...
  return (
    <>
      ...
      {
        productsFinal.length ? <Categories
      ...
      {
        filteredItem.length ? <Item 
        filteredItem={filteredItem}
        currentVariationID={currentVariationID}
        imgURL={imgURL}
        updateCurrentVariationID={updateCurrentVariationID}
        filteredVariation={filteredVariation}
